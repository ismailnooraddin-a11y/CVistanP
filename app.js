...
function renderChrome(activePage) {
  ...
  document.getElementById('chrome-header').innerHTML = `
    <header class="app-header">
      ...
      <div class="header-search">
        ...
        <input type="text" placeholder="Search patient, MRN, bed…" />
      </div>
  ...
  if (sidebar) {
    sidebar.innerHTML = `
      <div class="group-label">Clinical</div>
      <div class="nav-item ${activePage==='dashboard'?'active':''}" onclick="location.href='dashboard.html'">
        ${icon('grid')} ICU Dashboard
        <span class="badge">3</span>
      </div>
      <div class="nav-item">${icon('user')} My Patients</div>
      <div class="nav-item">${icon('clipboard')} Orders</div>
      <div class="nav-item">${icon('flask')} Results & Labs</div>
      <div class="nav-item">${icon('pill')} Medications</div>

      <div class="group-label">Workflow</div>
      <div class="nav-item">${icon('alert')} Alerts</div>
      <div class="nav-item">${icon('message')} Handovers</div>
      <div class="nav-item">${icon('calendar')} Schedule</div>

      <div class="group-label">System</div>
      <div class="nav-item">${icon('chart')} Analytics</div>
      <div class="nav-item">${icon('settings')} Settings</div>
    `;
  }
