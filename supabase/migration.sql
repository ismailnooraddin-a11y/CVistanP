...
CREATE TABLE IF NOT EXISTS resume_volunteer (
  ...
  organization TEXT NOT NULL DEFAULT '',
  category TEXT NOT NULL DEFAULT 'volunteer' CHECK (category IN ('volunteer', 'club', 'competition', 'student-org', 'community', 'other')),
  location TEXT,
  start_month INT,
  start_year INT,
  end_month INT,
...
CREATE INDEX idx_resume_volunteer_resume ON resume_volunteer(resume_id);
...
CREATE TABLE IF NOT EXISTS delivery_logs (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  resume_id UUID NOT NULL REFERENCES resumes(id) ON DELETE CASCADE,
  delivery_type TEXT NOT NULL CHECK (delivery_type IN ('download', 'email')),
  target TEXT,
...
-- Guest sessions: service role only (accessed via API routes with service key)
-- No user-facing RLS policy - accessed exclusively through server-side API routes
-- SECURITY: Ensure SUPABASE_SERVICE_ROLE_KEY is never exposed to client-side code
CREATE POLICY "Service role only for guest_sessions" ON guest_sessions
...
CREATE POLICY "Users manage own volunteer" ON resume_volunteer
  FOR ALL USING (resume_id IN (SELECT id FROM resumes WHERE user_id = auth.uid()));
